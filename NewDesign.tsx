...
import { Asset, Transaction, PerformanceData, ManagerProfile, ArbitrageStep } from './types';
...
const App: React.FC = () => {
    ...
    // Simulação do Motor Sniper (Realismo de Ganhos/Perdas)
    useEffect(() => {
        let interval: any;
        if (botActive) {
            interval = setInterval(() => {
                // Simulação: 20% de chance de uma operação de custo (taxas sem lucro)
                const isLoss = Math.random() < 0.20;
                const value = Number((Math.random() * 0.09 + 0.005).toFixed(4));

                const newStep: ArbitrageStep = {
                    id: Math.random().toString(36).substr(2, 9),
                    timestamp: new Date().toLocaleTimeString(),
                    path: isLoss ? ["USDT", "POL", "USDT"] : ["USDT", "WMATIC", "USDC", "USDT"],
                    profit: isLoss ? -value : value,
                    status: isLoss ? 'EXPIRED' : 'SUCCESS',
                    hash: '0x' + Math.random().toString(16).substr(2, 40)
                };

                if (isLoss) {
                    setDailyLoss(prev => prev + value);
                } else {
                    setDailyProfit(prev => prev + value);
                }

                setArbitrageLogs(prev => [newStep, ...prev].slice(0, 15));
            }, 2500);
        } else {
            setArbitrageLogs([]);
        }
        ...
        <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col md:flex-row font-['Inter'] selection:bg-[#f01a74]/30">
            ...
            <aside className="w-72 border-r border-zinc-800/50 hidden md:flex flex-col p-6 sticky top-0 h-screen bg-[#0c0c0e]">
                <div className="flex items-center gap-3 mb-10">
                    ...
                    <span className="font-bold text-2xl tracking-tighter bg-gradient-to-r from-white to-zinc-500 bg-clip-text text-transparent italic">FLOWSNIPER</span>
                </div>
